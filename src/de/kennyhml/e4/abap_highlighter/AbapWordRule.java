...
public class AbapWordRule implements IRule {
	 ...
	 * token associated with the detected word. If no token has been associated, the
	 * scanner will be rolled back and an undefined token will be returned in order
	 ...
	 */
	public AbapWordRule(IWordDetector detector) {
	 ...
	 * @see #addWord(String, IToken)
	 */
	public AbapWordRule(IWordDetector detector, IToken defaultToken) {
	 ...
	 * @since 3.3
	 */
	public AbapWordRule(IWordDetector detector, IToken defaultToken, boolean ignoreCase) {
	...
	public IToken evaluate(ICharacterScanner scanner) {
		...
		if (c != ICharacterScanner.EOF && fDetector.isWordStart((char) c)) {
			if (fColumn == UNDEFINED || (fColumn == scanner.getColumn() - 1)) {
				...
				if (fIgnoreCase)
					fLastWord = fLastWord.toLowerCase();

				IToken token = fWords.get(fLastWord);
